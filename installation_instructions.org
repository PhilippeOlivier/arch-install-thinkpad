...
Enable [Backspace] to go to previous page: about:config > browser.action_backspace 0

Import bookmarks: Library > Bookmarks > Show All Bookmarks > Import and Backup > Import Bookmarks from HTML

Start Firefox docked:
> sudo pacman -S alltray
- Add Firefox to Application Autostart (Name: Firefox; Command: alltray firefox)
...
- Browser: Firefox
- Mute sounds: ON
- Preferences > Conversations > Highlight misspelled words: OFF
...
Start Emacs docked:
- Add Emacs to Application Autostart (Name: Emacs; Command: alltray emacs)
...
- Show Transmission icon in the notification area: ON
...
Download the entrust_g2_ca.cer certificate and change the extention to .pem.

Network Connections > Add a new connection > Cisco AnyConnect Compatible VPN (openconnect):
Connection name: Polymtl VPN
VNP Protocol: Cisco AnyConnect
Gateway: ssl.vpn.polymtl.ca
CA Certificate: entrust_g2_ca.pem
...
** Terminal
Edit > Preferences > Appearance > Display menubar in new windows: OFF
...
** Other packages
*** Arch
dosfstools exfat-utils ntfs-3g (Microsoft formats)
...
onboard (virtual keyboard, just in case)
openssh sshpass (secure shell)
python-matplotlib python-networkx python-numpy python-pip python2-pip python-scipy python-unidecode (Python libraries and modules)
python2-lxml pstoedit (textext dependencies, for Inkscape)
qpdf (PDF manipulation)
sbcl (Common Lisp)
auctex texlive-core texlive-bin texlive-most (LaTeX)
...
textext (LaTeX extension for Inkscape)
...
Change layout option: Left Alt + Left Shift
...
- Preferences > Security > Junk make sure "Enable adaptive junk filter logging" is unchecked.
- Preferences > Advanced > General make sure that "Message Store Type for new accounts" is set to "File per message (maildir)".
- Preferences > Privacy > Tell sites that I do not want to be tracked: ON
- Preferences > Composition > Enable spell check as you type: OFF

Add-ons:
- Manually sort folders (reorder mailboxes)
NOTE: Enabling add-ons which are "incompatible": Preferences > Preferences > Advanced > General > Config Editor > extensions.strictCompatibility == false

Start Thunderbird docked:
- Add Thunderbird to Application Autostart (Name: Thunderbird; Command: alltray thunderbird)
